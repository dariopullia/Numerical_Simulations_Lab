...
//Read input informations
  ReadInput.open(mode+stato+"/input.in");
...
// distance i-j in pbc
      ...
      dy = Pbc(y[i] - y[j]);
      dz = Pbc(z[i] - z[j]);
//Mi aspetto di dover inserire cose


      dr = dx*dx + dy*dy + dz*dz;
      dr = sqrt(dr);
      if(dr < box/2){
        int bin=100*(dr/(0.5*box));
        Gr_block[bin] +=2;
      }
      if(dr < rcut)
      {
  ...
  walker[iv] = 4.0 * v+tailC_U; // Potential energy with tail corrections
  ...
  walker[ipr] =  rho*temp + appo_per_pres/(3.0*vol)+tailC_P;  // Pressure with tail corrections;
  return;
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, GR;
   //const int wd=12;
    
    cout << "Block number " << iblk << endl;
    cout << "Acceptance rate " << accepted/attempted << endl << endl;
    
    Epot.open(mode+stato+"/output_epot.dat",ios::app);
    Ekin.open(mode+stato+"/output_ekin.dat",ios::app);
    Temp.open(mode+stato+"/output_temp.dat",ios::app);
    Etot.open(mode+stato+"/output_etot.dat",ios::app);
    Pres.open(mode+stato+"/output_pres.dat",ios::app);    
    GR.open(mode+stato+"/output_Gr.dat",ios::app);    
...
void ConfFinal(void)
{
  ...
  WriteConf.open(mode+stato+"/config.out");
  WriteVelocity.open(mode+stato+"/velocity.out");
  for (int i=0; i<npart; ++i)
  {
...
void ConfXYZ(int nconf){ //Write configuration in .xyz format
  ...
  WriteXYZ.open(mode+stato+"/frames/config_" + to_string(nconf) + ".xyz");
  WriteXYZ << npart << endl;
